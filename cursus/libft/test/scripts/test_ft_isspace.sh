...
print_header "ft_isspace (Static Analysis + Logic)"
...
cc -Wall -Wextra -Werror -fanalyzer -c ../../ft_isspace.c -o ft_isspace.o

if [ $? -ne 0 ]; then
    print_error "Static Analysis found potential issues in ft_isspace.c"
    exit 1
else
...
cc -Wall -Wextra -Werror ../src/test_isspace_runner.c -L../.. -lft -I../.. -o test_isspace
...
TEST_CASES=(
    "32" "9" "10" "11" "12" "13" "65" "48" "0" "-1"
)

FAILED=0
for test in "${TEST_CASES[@]}"; do
    # Run logic test with Valgrind
    run_with_valgrind "test_isspace" "$test"
    RES=$?
    if [ $RES -eq 42 ]; then
...
# Cleanup
rm -f test_isspace ft_isspace.o

print_summary "ft_isspace" $FAILED
exit $FAILED
