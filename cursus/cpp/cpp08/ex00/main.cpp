...
void testContainer(Container& c, int valueToFind,
                   const std::string& containerName) {
  std::cout << CYAN << "Testing " << containerName
            << " - Searching for: " << valueToFind << RESET << std::endl;
  try {
    typename Container::iterator it = easyfind(c, valueToFind);
    std::cout << GREEN << "Success! Element " << *it << " found." << RESET
              << std::endl;
...
int main() {
  std::cout << YELLOW << "\n========== TEST: STD::VECTOR ==========" << RESET
            << std::endl;
  std::vector<int> vec;
  for (int i = 0; i < 5; ++i) vec.push_back(i * 10);  // 0, 10, 20, 30, 40
