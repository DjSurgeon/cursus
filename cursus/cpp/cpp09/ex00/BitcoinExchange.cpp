...
void BitcoinExchange::processInput(const std::string& inputPath) const {
    ...
    while (std::getline(file, line)) {
         ...
         * =========================================================================================
         * The `std::map` container is internally implemented as a self-balancing binary search tree
         * (typically a Red-Black tree). This means that the keys are kept strictly ordered lexicographically (which fits perfectly with 
         * dates in YYYY-MM-DD format).
         * 
         * The `lower_bound(key)` method searches algorithmically in O(log N) time for the first iterator 
         * that points to an element whose key is NOT strictly less than `key`. That is, the found element will be `>= key`.
         * 
         * To comply with the subject rule: "If the date does not exist, use the nearest previous date", 
         * the logical flow using iterators is as follows:
         * 
         * 1. We perform the search: `it = _database.lower_bound(dateStr)`.
         * 2. If `it` points exactly to `dateStr`, we use that value (the exact date was found).
         * 3. If `it` is NOT the exact date, it means it points to the first immediately **POSTERIOR** date 
         *    a `dateStr` in the database (or `.end()` if `dateStr` is greater than all).
         * 4. To get the "nearest previous date", we simply move the iterator back one position: `--it`.
         * 5. **CRITICAL edge case**: If `it == _database.begin()` and it does not match the exact date, 
         *    then `dateStr` is older than any record in our CSV, so it is logically impossible 
         *    to offer a valid historical valuation.
         * =========================================================================================
         */
        ...
        if (it == _database.end() || it->first != dateStr) {
            if (it == _database.begin()) {
                std::cerr << "Error: Date " << dateStr << " is older than any record in the database." << std::endl;
                continue;
            }
